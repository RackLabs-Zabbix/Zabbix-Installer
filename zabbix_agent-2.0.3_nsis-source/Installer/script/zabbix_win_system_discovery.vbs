...
'SQL NAMED INSTANCE PROPS
PROP_SQLNAME      = 0
PROPNAME_SQLNAME       = "SQLINSTNAME"

'GET SQL INSTANCES
function sql_server_instances()
    aryInstances=array
	sqlver=Array("ComputerManagement","ComputerManagement10","ComputerManagement11")

	For Each sql in sqlver
		On Error Resume next
		Set objWMIService = GetObject("winmgmts:\\.\root\Microsoft\SqlServer\" & sql) 
		If Err= 0 Then
			Set colItems = objWMIService.ExecQuery( _
			"SELECT ServiceName FROM SqlService WHERE SQLServiceType = 1",,48) 
			For Each objItem in colItems 
'				If not IsNull(aryInstances(0)) then
				redim preserve aryInstances(ubound(aryInstances)+1)
'				End if
				If InStr(objItem.ServiceName,"MSSQLSERVER") Then
					objItem.ServiceName = "SQLServer"
				End If
				aryInstances(ubound(aryInstances))=objItem.ServiceName
			Next
		End If
		Err.Clear
	Next
...
ElseIf (StrComp(strOption, OPT_NICS, 1) = 0) Then	
	...
	'DEFINE JSON PAIRS
	Dim NicPairs()	'NICNAME, NICDEV, NICDEVNUM, NICPERF, NICMAC, NICSTATE

	'GET NICS
	Set colNics = objWMI.ExecQuery("Select * from Win32_NetworkAdapter")	
	ReDim NicPairs(6 - 1, colNics.Count - 1)
...
'GET SQL INSTANCES
ElseIf (StrComp(strOption, OPT_SQLNAME, 1) = 0) Then
	Dim SQLInstancePairs()
	dim aryInstances, intInstance
	dim RowCount
	aryInstances = sql_server_instances()
	if not isempty(aryInstances) then
		RowCount = ubound(aryInstances)
		ReDim SQLInstancePairs(1 - 1, RowCount)
		for intInstance=0 to RowCount
		    SQLInstancePairs(PROP_SQLNAME,intInstance) = PROPNAME_SQLNAME & "|" & aryInstances(intInstance)
		next
	else
		'
	end if
	' MakeJSON unknown
	strResult = MakeJSON(SQLInstancePairs, RowCount+1)
